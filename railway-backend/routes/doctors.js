...
router.get("/", async (req, res) => {
  ...
  const where  = isAdmin ? [] : [
    "d.is_active = TRUE",
    // Hide lapsed licences from citizens (30-day grace period). Legacy rows
    // with a NULL licence_expiry_date are still shown — they predate the
    // application pipeline and were manually added by a super_admin.
    "(d.licence_expiry_date IS NULL OR d.licence_expiry_date > DATE_SUB(NOW(), INTERVAL 30 DAY))",
  ];
