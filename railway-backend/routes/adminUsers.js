...
router.get("/", async (req, res) => {
  ...
  // Vet / Shop applicants are stored in `users` with role='citizen' (that's
  // the only role the public /api/business/register endpoint may create — see
  // routes/business.js). Semantically they are NOT pet-owner citizens: they
  // already appear in the Listings tab (pending) and the Doctors / Shops
  // tabs (once approved). Exclude anyone with a linked business_applications
  // row from the Citizen list so a vet doesn't show up as a "Citizen".
  // When the caller explicitly asks for a non-citizen role we skip this so
  // it's a no-op for admin lists.
  const listingCitizens = !role || role === "citizen";
  if (listingCitizens) {
    where.push(`NOT EXISTS (
      SELECT 1 FROM business_applications ba WHERE ba.applicant_id = u.id
    )`);
  }
